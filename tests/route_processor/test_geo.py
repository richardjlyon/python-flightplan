...
def test_length(montrose_to_forfar):
    """Tests that the length of the `montrose_to_forfar` segment is accurately calculated."""
    segment = montrose_to_forfar
    assert segment.length == pytest.approx(15.4, abs=0.1)


def test_true_bearing(montrose_to_forfar):
    """Tests that the length of the `montrose_to_forfar` segment is correctly calculated."""
    segment = montrose_to_forfar
    assert segment.true_bearing == 254


def test_magnetic_bearing(montrose_to_forfar):
    """Tests that the bearing of the `montrose_to_forfar` segment is correctly calculated."""
    segment = montrose_to_forfar
    assert segment.magnetic_bearing == 253


def test_travel_time(montrose_to_forfar):
    """Tests that the travel time of the `montrose_to_forfar` segment is correctly calculated."""
    segment = montrose_to_forfar
